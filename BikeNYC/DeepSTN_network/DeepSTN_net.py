...
import BikeNYC.DeepSTN_network.metrics as metrics
...
def Res_plus(name, F, Fplus, rate, drop, H, W):
    ...
    if rate == 1:
        cl_conv1B = cl_input
    if rate != 1:
        cl_conv1B = AveragePooling2D(pool_size=(rate, rate), strides=(rate, rate), padding="valid")(
            cl_input)

    cl_conv1B = Activation('relu')(cl_conv1B)
    cl_conv1B = BatchNormalization()(cl_conv1B)

    plus_conv = Conv2D(filters=Fplus * H * W,
                       kernel_size=(int(np.floor(H / rate)), int(np.floor(W / rate))),
                       padding="valid")
...
def DeepSTN(H=21, W=12, channel=2,  # H-map_height W-map_width channel-map_channel
    ...
    c_input = Lambda(cpt_slice, arguments={'h1': cut0, 'h2': cut1})(cpt_input_p)
    p_input = Lambda(cpt_slice, arguments={'h1': cut1, 'h2': cut2})(cpt_input_p)
    t_input = Lambda(cpt_slice, arguments={'h1': cut2, 'h2': cut3})(cpt_input_p)
    ...
    if is_pt:
        ...
        if kernel1:
            ...
            cpt = conv_unit0(pre_F * 3 + PT_F * isPT_F + P_N * (not isPT_F), conv_F, drop, H, W)(
                cpt_con1)
